...
def simulate(job_id, num_jobs, sim_suffix):
    Be_rng_comb, Bi_rng_comb, EE_probchg_comb, EI_probchg_comb, II_condchg_comb, E_extra_comb, bkg_chg_comb, C_rng_comb, evfr_chg_comb = np.meshgrid(Be_rng, Bi_rng, EEconn_chg_factor, EIconn_chg_factor, IIconn_chg_factor, E_extra_stim_factor, bkg_chg_factor, C_rng, evoked_fr_chg_factor)
    ...
    for ij1 in range(Be_rng_comb.size):
        ...
        for nn_stim in nn_stim_rng:
            ...
            rr1_e = np.hstack((r_bkg_e*np.ones(NE), r_bkg_e*np.ones(NI)))
            rr1_i = np.hstack((r_bkg_i*np.ones(NE), r_bkg_i*np.ones(NI)))
            rr1 = [rr1_e, rr1_i]
        ...
        # -- result path
        sim_suffix_comp = sim_suffix.format(r_bkg_e/1.e3, r_bkg_i/1.e3, ev_arrange, pert_type, N, evfr_chg_comb[ij1]*100, ev_prop*100, ev_pop, C_rng.size, Ntrials, stim_inh/1.e3, stim_exc/1.e3, r_act_inh*EI_probchg_comb[ij1], r_act_exc*EE_probchg_comb[ij1], II_scale, bkg_chg_comb[ij1], p_ItoI)
