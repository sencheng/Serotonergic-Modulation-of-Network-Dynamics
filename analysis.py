...
reload(searchParams);
from searchParams import C_rng
...
def _adapt_size(data, arr):
    if arr.size > 1:
        return data[arr].mean(axis=0)
    elif arr.size == 1:
        return data[arr].reshape((data.shape[1],))
    else:
...
class simdata():
    ...
    def __init__(self, fl_path):
        ...
        self.ev_resp_exc = {}
        self.ev_resp_inh = {}
        self.baseline_change_exc = {}
        self.baseline_change_inh = {}
        ...
        self.w_etoe = self.data['W_EtoE']
        self.w_etoi = self.data['W_EtoI']
    ...
    def _get_ind_spike_count(self, pert, interval):
    ...
    def _count_input_spikes(self, pert, interval):
        fr = np.mean(self._get_ind_spike_count(pert, interval), axis=1)
        fr_exc = fr[:NE]
        fr_inh = fr[NE:]
        num_exc_inp_spikes = np.zeros(N)
        num_inh_inp_spikes = np.zeros(N)
        for n_id in range(0, NE):
            w = self.w_etoe[:, n_id]
            num_exc_inp_spikes[n_id] = np.sum(fr_exc[np.abs(w) > 0])
            w = self.w_itoe[:, n_id]
            num_inh_inp_spikes[n_id] = np.sum(fr_inh[np.abs(w) > 0])
        for n_id in range(NI):
            w = self.w_etoi[:, n_id]
            num_exc_inp_spikes[n_id + NE] = np.sum(fr_exc[np.abs(w) > 0])
            w = self.w_itoi[:, n_id]
            num_inh_inp_spikes[n_id + NE] = np.sum(fr_inh[np.abs(w) > 0])
        return num_exc_inp_spikes, num_inh_inp_spikes

    def plot_input_rate_dist(self, pert, interval, ax):
        exc_inp_count, inh_inp_count = self._count_input_spikes(pert, interval)
        cnt_etoe, e_etoe = np.histogram(exc_inp_count[:NE], 20)
        cnt_itoe, e_itoe = np.histogram(inh_inp_count[:NE], 20)
        e_etoe = (e_etoe[:-1] + e_etoe[1:]) / 2
        e_itoe = (e_itoe[:-1] + e_itoe[1:]) / 2
        ax[0].plot(e_etoe, cnt_etoe, color='red',
                   label=r'$r_E={:.0f}$'.format(exc_inp_count[:NE].mean()))
        ax[0].plot(e_itoe, cnt_itoe, color='blue',
                   label=r'$r_I={:.0f}$'.format(inh_inp_count[:NE].mean()))
        ax[0].set_title('excitatory neurons')
        ax[0].legend()

        cnt_etoi, e_etoi = np.histogram(exc_inp_count[NE:], 20)
        cnt_itoi, e_itoi = np.histogram(inh_inp_count[NE:], 20)
        e_etoi = (e_etoi[:-1] + e_etoi[1:]) / 2
        e_itoi = (e_itoi[:-1] + e_itoi[1:]) / 2
        ax[1].plot(e_etoi, cnt_etoi, color='red',
                   label=r'$r_E={:.0f}$'.format(exc_inp_count[NE:].mean()))
        ax[1].plot(e_itoi, cnt_itoi, color='blue',
                   label=r'$r_I={:.0f}$'.format(inh_inp_count[NE:].mean()))
    ...
    def _get_ind_fr_pert_by_mdl(self, pert, dt):
        time_edges = np.arange(0, self.evoke_interval[1] + 1.1 * dt, dt)
        time_vec = (time_edges[1:] + time_edges[0:-1]) / 2
        pop_fr = np.zeros((C_rng.size, N, time_vec.size))
        for mdl in range(0, C_rng.size):
            ids = []
            spk_times = []
            for tr in range(0, Ntrials):
                ids.append(self.data[pert][2][tr + mdl * Ntrials]['senders'])
                spk_times.append(self.data[pert][2][tr + mdl * Ntrials]['times'])
            ids = np.concatenate(ids)
            spk_times = np.concatenate(spk_times)
            for n_id in range(0, N):
                sel_spk_times = spk_times[(ids == n_id + 1)]
                pop_fr[mdl, n_id] = np.histogram(sel_spk_times, time_edges)[0] / dt * 1000 / Ntrials
        return {'exc': pop_fr[:, :NE], 'inh': pop_fr[:, NE:], 'time': time_vec}

    def _get_ind_fr_pert(self, pert, dt):
        ids = []
        spk_times = []
        ...
        time_vec = (time_edges[1:] + time_edges[0:-1]) / 2
        pop_fr = np.zeros((N, time_vec.size))
        for n_id in range(0, N):
            sel_spk_times = spk_times[(ids == n_id + 1)]
            pop_fr[n_id] = smooth(np.histogram(sel_spk_times, time_edges)[0] / dt * 1000 / self.Ntrials)
        return pop_fr[:NE], pop_fr[NE:], time_vec

    def _get_pop_fr_pert_subset(self, pert, dt, inds):
        exc_fr, inh_fr, time_vec = self._get_ind_fr_pert(pert, dt)
        exc_fr = _adapt_size(exc_fr, inds[pert]['exc'] - 1)
        inh_fr = _adapt_size(inh_fr, inds[pert]['inh'] - 1 - NE)
        return exc_fr, inh_fr, time_vec
    ...
    def get_inc_dec_ids(self, intervals):
        for pert in nn_stim_rng:
            fr_0 = self._get_ind_spike_count(pert, intervals[0]).mean(axis=1)
            fr_1 = self._get_ind_spike_count(pert, intervals[1]).mean(axis=1)
            dfr = fr_1 - fr_0
            inc_ids = np.where(dfr > 0)[0] + 1
            dec_ids = np.where(dfr < 0)[0] + 1
            neu_ids = np.where(dfr == 0)[0] + 1
            self.inc_dec_sub_ids['inc'][pert] = {'exc': inc_ids[inc_ids <= NE],
    ...
    def get_baseline_change(self):
        ...
        fr_stim_diff_inh = self.baseline_change_inh.copy()
        for pert in nn_stim_rng:
            fr_base = self._get_gain_pert(pert, self.base_interval + np.array([20., 0.]))
            fr_stim = self._get_gain_pert(pert, self.stim_interval + np.array([20., 0.]))
            fr_diff = fr_stim - fr_base
            self.baseline_change_exc[pert] = fr_diff[self.ids_included[self.ids_included <= NE] - 1].mean()
            self.baseline_change_inh[pert] = fr_diff[self.ids_included[self.ids_included > NE] - 1].mean()
            fr_stim_diff_exc[pert] = (fr_stim[self.ids_included[self.ids_included <= NE] - 1].mean() /
                                fr_base[self.ids_included[self.ids_included <= NE] - 1].mean())
            fr_stim_diff_inh[pert] = (fr_stim[self.ids_included[self.ids_included > NE] - 1].mean() /
                                fr_base[self.ids_included[self.ids_included > NE] - 1].mean())
    ...
    def plot_subpop_fr(self, ax, dec_inc='inc'):
        print("\nPlotting the subpopulation firing rates\n")
        if dec_inc == 'inc':
            arr_inh = self.fr_inh_inc
            arr_exc = self.fr_exc_inc
        elif dec_inc == 'dec':
        ...
        if isinstance(ax, np.ndarray):
            for ax_ind, ax_ in enumerate(ax):
                ax_.plot(self.fr_exc['time'],
                         arr_exc[nn_stim_rng[ax_ind]],
                         color='red', linewidth=2, label='Exc.')
                ax_.plot(self.fr_inh['time'],
                         arr_inh[nn_stim_rng[ax_ind]],
                         color='blue', linewidth=2, label='Inh.')
                ax_.set_xlabel('Time (ms)')
                if nn_stim_rng[ax_ind] == 0:
                    title = 'control'
            ...
            ax.plot(self.fr_exc['time'],
                    arr_exc[nn_stim_rng[0]],
                    color='red', linewidth=2, label='Exc.')
            ax.plot(self.fr_inh['time'],
                    arr_inh[nn_stim_rng[0]],
                    color='blue', linewidth=2, label='Inh.')
            ax.set_xlabel('Time (ms)')
            ax.set_ylabel('Firing rate (spk/s)')
            ax.legend()
    ...
    def plot_trbytr_ind_mem_pots(self, ax):
        ...
        for p_i, pert in enumerate(nn_stim_rng):
            print("\nPlotting individual membrane potentials for individual trials for pert={:.0f}%".format(
                pert / NI * 100))
            ids = self.data[pert][-1][0]['senders']
            u_ids = np.unique(ids)
            if pert == 0:
    ...
    def plot_avg_conds_all(self, ax):

        self._get_cond_pert_all()
        for p_i, pert in enumerate(nn_stim_rng):
            print("\nPlotting the average conductances for pert={:.0f}%".format(pert / NI * 100))
            for ei_i, ex_in in enumerate(self.conds[pert].keys()):
                conds = self.conds[pert][ex_in]
                if len(conds.shape) > 1:
                    conds = conds.mean(axis=0)
                ax[p_i].plot(self.conds['times'], conds,
                             color=self.color_dict[ex_in],
                             label=ex_in,
                             linewidth=1)
                # if p_i == nn_stim_rng.size-1:
                ax[p_i].set_xlabel('Times (ms)')
